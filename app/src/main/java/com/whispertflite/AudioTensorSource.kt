...
class AudioTensorSource {
    companion object {
        ...
        fun fromRawPcmBytes(rawBytes: ByteArray): OnnxTensor {
            ...
            Log.d(MainActivity.TAG, "AudioRecord.read(float[], ...) returned $numSamples")
        ...
        @JvmStatic
        fun fromRawWavfile(filePath: String): OnnxTensor {
            val floatSamples = WaveUtil.getSamples(filePath);
            if (floatSamples.size > maxAudioLengthInSeconds * sampleRate) {
                val numSamples = maxAudioLengthInSeconds * sampleRate;
                val newfloatSamples = floatSamples.copyOf(numSamples);
                val floatBuffer = FloatBuffer.wrap(newfloatSamples);
                ...
                val floatBuffer = FloatBuffer.wrap(floatSamples);
                val numSamples = floatBuffer.capacity();
        ...
        @JvmStatic
        fun fromRawSamples(floatSamples : FloatArray): OnnxTensor {
            var floatBuffer = FloatBuffer.wrap(floatSamples);
//            val numSamples = minOf(floatBuffer.capacity(), maxAudioLengthInSeconds * sampleRate)
            var numSamples = floatBuffer.capacity();
            if (numSamples > maxAudioLengthInSeconds * sampleRate) {
                numSamples = maxAudioLengthInSeconds * sampleRate;
                floatBuffer = resizeBuffer(floatBuffer, numSamples);
            }
        ...
        private fun resizeBuffer(floatBuffer: FloatBuffer, numSamples: Int): FloatBuffer {
            val resizedBuffer = FloatBuffer.allocate(numSamples)
            floatBuffer.flip()
            resizedBuffer.put(floatBuffer)
